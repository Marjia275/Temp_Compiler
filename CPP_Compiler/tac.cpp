...
std::string TACGenerator::genExpr(ASTNode* node) {
    switch (node->type) {
        ...
        case NODE_COUT:      return "cout";
        case NODE_CIN:       return "cin";
        case NODE_ENDL:      return "endl";
        ...
        case NODE_SIZEOF: {
            string t = newTemp();
            string what = node->datatype.empty() ? node->sval : node->datatype;
            emit(t + " = sizeof(" + what + ")");
            return t;
        }

        case NODE_TERNARY: {
            string cond = genExpr(node->children[0]);
            string Lelse = newLabel();
            string Lend = newLabel();
            string t = newTemp();
            emit("if " + cond + " == 0 goto " + Lelse);
            string thenVal = genExpr(node->children[1]);
            emit(t + " = " + thenVal);
            emit("goto " + Lend);
            emit(Lelse + ":");
            string elseVal = genExpr(node->children[2]);
            emit(t + " = " + elseVal);
            emit(Lend + ":");
            return t;
        }
...
void TACGenerator::genStatement(ASTNode* stmt) {
    switch (stmt->type) {
        ...
        case NODE_DOWHILE: {
            string Lstart = newLabel();
            string Lcontinue = newLabel();
            string Lend = newLabel();
            emit(Lstart + ":");
            loopLabels.push_back({Lcontinue, Lend});
            genBlock(stmt->children[0]);
            loopLabels.pop_back();
            emit(Lcontinue + ":");
            string cond = genExpr(stmt->children[1]);
            emit("if " + cond + " != 0 goto " + Lstart);
            emit(Lend + ":");
            break;
        }
        ...
        default:
            // Bare expression statement, e.g. "cout << a << endl;"
            genExpr(stmt);
            break;
    }
}

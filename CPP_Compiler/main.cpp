// ============================================================
// main.cpp
// Driver for the Mini C Compiler.
// Controls the whole pipeline exactly like the calculator's
// main(), just with more phases:
//
//   Open input.cpp
//     -> Lexical Analysis   (debug only: token dump)
//     -> yyparse()          (builds the AST)
//     -> AST                (debug only: tree dump)
//     -> Semantic Analysis
//     -> TAC / Codegen / Assembly   (debug only: educational dump)
//     -> Interpreter                (ALWAYS runs -> real program output)
//     -> Symbol Table       (debug only: final values)
//
// Usage:
//   compiler.exe input.cpp            -> prints ONLY the program's output
//   compiler.exe input.cpp --debug    -> prints every compiler phase too
// ============================================================
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, char** argv) {
    // ---------------- Figure out mode: file vs. pasted stdin ----------------
    // Usage:
    //   compiler.exe                 -> paste code, run it, show only output
    //   compiler.exe --debug         -> paste code, show every phase
    //   compiler.exe input.cpp         -> read from file, show only output
    //   compiler.exe input.cpp --debug -> read from file, show every phase
    bool debug = false;
    ...
    if (argc == 1) {
        ...
        cerr << "Usage: " << argv[0] << " [input.cpp] [--debug]" << endl;
        return 1;
    }
    ...
    if (filename.empty()) {
        ...
        cerr << "Paste your C++ code below, then press Ctrl+Z and Enter (Windows)"
             << endl
    ...
    if (parseResult != 0 || !g_syntaxOk || astRoot == nullptr) {
        cout << "Compilation stopped due to syntax error(s) above." << endl;
        return 1;
    }

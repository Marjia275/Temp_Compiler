...
std::vector<std::string> AssemblyGenerator::generate(const std::vector<std::string>& tac) {
    ...
    for (const string& line : tac) {
        ...
        if (line.rfind("println", 0) == 0) {
            if (line.size() > 8) {
                string arg = line.substr(8);
                if (!arg.empty() && arg.front() == '"') out.push_back("; push string " + arg);
                else out.push_back("MOV eax, " + arg);
                out.push_back("CALL System.out.print");
            }
            out.push_back("CALL System.out.println_newline");
            continue;
        }
        ...
        if (eqPos != string::npos) {
            ...
            if (toks.size() == 1) {
                ...
                if (toks[0] == "-") out.push_back("NEG eax");
                else if (toks[0] == "~") out.push_back("NOT eax");
                else if (toks[0] == "!") out.push_back("CMPEQ eax, 0");
                out.push_back("MOV " + dest + ", eax");
            } else if (toks.size() == 3) {
                out.push_back("MOV eax, " + toks[0]);

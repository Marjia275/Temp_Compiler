...
bool SymbolTable::declare(const std::string& name, const std::string& type, bool isFinal) {
...
void SymbolTable::print() const {
    ...
    for (auto& scope : scopes) {
        for (auto& pair : scope) {
            ...
            cout << left << setw(10) << s.name << setw(10) << s.type;
            if (s.type == "int")               cout << setw(15) << s.value.ival;
            else if (s.type == "float" || s.type == "double") cout << setw(15) << s.value.fval;
            else if (s.type == "char")         cout << setw(15) << s.value.cval;
            else if (s.type == "boolean")      cout << setw(15) << (s.value.ival ? "true" : "false");
            else if (s.type == "String")       cout << setw(15) << s.value.sval;
            else if (s.type == "Scanner")      cout << setw(15) << "(scanner)";
